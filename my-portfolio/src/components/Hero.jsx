// src/components/HeroWithStats.jsx
import React from "react";
import { useState, useRef } from "react";
import { Typewriter } from "react-simple-typewriter";
import { gsap } from "gsap";
import { useGSAP } from "@gsap/react";


export default function HeroWithStats() {
    const [currentWord, setCurrentWord] = useState("Full Stack");
    const avatarRef = useRef(null);

    useGSAP(() => {
        gsap.to(avatarRef.current, {
            y: -9,         // float up by 10px
            duration: 2,    // seconds per float cycle
            yoyo: true,
            repeat: -1,
            ease: "sine.inOut",
        });
    }, []);

    return (
        <div id="home" className="min-h-screen text-white">
            <div className="container mx-auto px-4 py-12">
                <div className="grid lg:grid-cols-2 gap-12 items-center">

                    {/* Content */}
                    <div className="order-2 lg:order-1 space-y-6">
                        <div className="font-mono text-gray-400 nameSpan">
                            {"<span>"}Hey, I'm Atharva Jadhav{"</span>"}
                        </div>

                        <h1 className="text-4xl lg:text-5xl font-semibold leading-tight text-white">
                            <span className="text-green-400 font-mono">{"{"}</span>
                            <span className="text-green-400 inline-block">
                                <Typewriter
                                    words={["Full Stack", "CSE @TSEC"]}
                                    loop={0}
                                    cursor
                                    cursorStyle="|"
                                    typeSpeed={70}
                                    deleteSpeed={50}
                                    delaySpeed={1000}
                                    onType={(count) =>
                                        setCurrentWord(
                                            ["Full Stack", "CSE @TSEC"][count % 2]
                                        )
                                    }
                                />
                            </span>
                            <span className="text-green-400 font-mono">{"}"}</span>
                            <br />
                            {currentWord === "Full Stack" ? (
                                <span>Web Developer</span>
                            ) : (
                                <span>Aspiring Software Engg</span>
                            )}
                            <span className="text-green-400">_</span>
                        </h1>


                        <div className="text-gray-400 font-mono text-sm leading-relaxed">
                            {"<p>"} With expertise in cutting-edge technologies such as{" "}
                            <span className="text-blue-400">NodeJS</span>,{" "}
                            <span className="text-blue-400">React</span>,{" "}
                            <span className="text-blue-400">Angular</span>, and{" "}
                            <span className="text-blue-400">Laravel</span>... I deliver web
                            solutions that are both innovative and robust. {"</p>"}
                        </div>

                        <div className="flex items-center space-x-4 text-gray-100">
                            <div className="w-6 h-6 rounded text-center" style={{ backgroundColor: "#082535" }}>M</div>
                            <div className="w-6 h-6 rounded text-center" style={{ backgroundColor: "#F7E029" }}>E</div>
                            <div className="w-6 h-6 rounded text-center" style={{ backgroundColor: "#67DBFB" }}>R</div>
                            <div className="w-6 h-6 rounded text-center" style={{ backgroundColor: "#6DA368" }}>N</div>
                        </div>
                    </div>
                    {/* Profile Image */}
                    <div className="order-1 lg:order-2 flex justify-center lg:justify-center py-5">
                        <div className="relative">
                            {/* <div className="w-95 h-95 bg-gradient-to-br from-gray-700 to-gray-800 transform rotate-45 rounded-3xl overflow-hidden">
                                <div className="w-full h-full transform -rotate-45 scale-150 translate-y-8" ref={avatarRef}> */}
                            <div className="w-65 h-65 md:w-full md:h-95 pt-3" ref={avatarRef}>
                                <div className="w-full h-full transform scale-112">
                                    <img
                                        src="/Firefly_Modern flat vector illustration of a young computer engineering student working on a  141617-Photoroom.png"
                                        alt="Atharva Jadhav Profile"
                                        className="w-full h-full object-cover"
                                    />
                                </div>
                            </div>
                            {/* </div>
                            </div> */}
                            <div className="absolute -bottom-4 -right-4 bg-green-500 p-3 rounded-lg">
                                <div className="text-gray-900 font-mono text-lg">{"</>"}</div>
                            </div>
                        </div>
                    </div>
                </div>

            </div>
        </div>
    );
}